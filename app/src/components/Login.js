...
function Login(props) {
  ...
  useEffect(
    () => {
      if (localStorage.getItem('token')) {//if token exists
        axios.post(process.env.REACT_APP_API_LINK + "/auth/validate", undefined, { //validate token API endpoint
          headers: {
  ...
  const handleClick = () => {
    ...
    fetch(process.env.REACT_APP_API_LINK + "/auth", //login API endpoint - sets token in local storage if successful
      {
        method: 'POST',
