...
function Study(props) {
    ...
    const handleEditSubmit = (event) => { // handle submission of an edited study
        ...
        axios.post(process.env.REACT_APP_API_LINK + "/study/update", formData, // post the form data to the update study endpoint
            {
                headers: {
                    'Content-Type': 'multipart/form-data',
                    'Authorization': 'Bearer ' + localStorage.getItem('token')
    ...
    useEffect(() => {
        fetch(process.env.REACT_APP_API_LINK + "/studies?short_name=" + short_name, {
            headers: {
                'Authorization': 'Bearer ' + localStorage.getItem('token')
            }
        })
        ...
        fetch(process.env.REACT_APP_API_LINK + "/files?short_name=" + short_name, {
            headers: {
                'Authorization': 'Bearer ' + localStorage.getItem('token')
            }
        })
    ...
    useEffect(() => {
        fetch(process.env.REACT_APP_API_LINK + "/statuses", {
            headers: {
                'Authorization': 'Bearer ' + localStorage.getItem('token')
            }
        })
                                            ...
                                            <Tabs defaultActiveKey="registration" id="pdfs">
                                                {fileNames.map((file) => (
                                                    <Tab key={file.fileName} eventKey={file.fileName} title={file.fileName}>
                                                        <Row>
                                                            <Col>
                                                                <a href={process.env.REACT_APP_API_LINK + "/fileuploads/" + file.fileName}>
